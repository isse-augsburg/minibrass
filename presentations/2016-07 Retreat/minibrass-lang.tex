...
\onslide<3->{  
\node[overlay,align=left,rectangle callout,%
      callout absolute pointer=(mbrlibs.west),fill=CornflowerBlue!50] at (-4.5,3.5) {PVS-Funktionen};} 
     
\onslide<2->{  
\node[overlay,align=left,rectangle callout,%
      callout absolute pointer=(mbr.north),fill=CornflowerBlue!50] at (-1,6.5) {PVS-Typen, Instanzen, Kombinationen};} 
...
\begin{frame}{Gewichte für Constraint Relationships}
...
\vspace*{1ex} \small
Für Ordnung $P$ über Constraints: PVS $\mathrm{Weighted}(P) = \langle \mathbb{N}, +, \geq, 0 \rangle$.

\vspace*{1ex}

\hfill \cite{Schiendorfer13}
\begin{textblock*}{3cm}[1,1](\textwidth+1cm,\textheight-2.3cm)
