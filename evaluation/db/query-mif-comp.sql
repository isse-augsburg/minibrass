...
where SPD = 1 and MIF = 1 and timeout > 10000 and PropRed = 0 and SolverId != 6
...
JobResult jr INNER JOIN Config cf ON jr.ConfigId = cf.ID 
INNER JOIN Solver sv ON jr.SolverId = sv.ID 
where SPD = 1 and MIF = 0 and timeout > 10000 and PropRed = 0 and SolverId != 6
order by problem, instance, elapsedSecs, solverId ;
...
md.elapsedSecs / nmd.elapsedSecs as rel,
...
--AVG(rel) as "Rel",
SUM(MifWins), COUNT(*), 
1.0* SUM(MifWins) / COUNT(*) as "RatioMifWins" 
FROM 
MifSummary
GROUP BY SolverName 
