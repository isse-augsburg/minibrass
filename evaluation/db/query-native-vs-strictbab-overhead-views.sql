...
SELECT problem, instance, SolverName, SolverId,elapsedSecs, Objective, Optimally, cf.ID as ConfigId FROM 
JobResult jr INNER JOIN Config cf ON jr.ConfigId = cf.ID 
INNER JOIN Solver sv ON jr.SolverId = sv.ID 
...
Select pd.Problem, pd.Instance, pd.SolverName, pd.elapsedSecs as "SmythElapsed", pd.Objective as "SmythObj", nd.elapsedSecs as "WeightsElapsed", td.elapsedSecs as "ToulbarElapsed", nd.Objective as "WeightsObjective" ,
