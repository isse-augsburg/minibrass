...
/**
 * Implements sum voting (which is only feasible if the supplied PVS are
 * integer) Agents may place any weight they like on a decision (which we can
 * then minimize or maximize)
 * 
...
public class SumVoting implements VotingProcedure {
	...
	public String getVotingPredicate(CodeGenerator codeGen, Collection<AbstractPVSInstance> votingPvs) {
		...
		for (AbstractPVSInstance voter : votingPvs) {
			...
			sb.append(codeGen.getOverallValuation(voter) + codeGen.getTopValue(voter));
		...
		for (AbstractPVSInstance voter : votingPvs) {
			...
			voter = codeGen.deref(voter);
			sb.append("sol(" + codeGen.getOverallValuation(voter) + codeGen.getTopValue(voter) + ")");
		}
		sb.append("])\n");
		return sb.toString();
	}
	...
	public String getNumericObjective(CodeGenerator codeGen, Collection<AbstractPVSInstance> votingPvs) {
		...
		for (AbstractPVSInstance voter : votingPvs) {
			...
			voter = codeGen.deref(voter);
			sb.append(codeGen.getOverallValuation(voter));
