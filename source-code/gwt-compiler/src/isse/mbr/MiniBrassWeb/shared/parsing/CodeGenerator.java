...
import isse.mbr.MiniBrassWeb.shared.model.parsetree.CompositePVSInstance;
import isse.mbr.MiniBrassWeb.shared.model.parsetree.MiniZincBinding;
...
import isse.mbr.MiniBrassWeb.shared.model.parsetree.ProductType;
...
import isse.mbr.MiniBrassWeb.shared.model.types.MiniZincArrayLike;
...
public class CodeGenerator {
	...
	private boolean suppressOutputGeneration = false; // this should not be accessible from outside, better use the new
														// MiniBrass output item
	...
	private void addIncludes(StringBuilder sb, MiniBrassAST model) {
		...
		for (Entry<String, PVSType> pvsType : model.getPvsTypes().entrySet()) {
			...
			if (!addedFiles.contains(referencedFile)) {
				sb.append("include \"" + referencedFile + "\";\n");
		...
		for (String addMinizincFile : model.getAdditionalMinizincIncludes()) {
			sb.append("include \"" + addMinizincFile + "\";\n");
		}
	}
	...
	private void addPvsInstances(StringBuilder sb, MiniBrassAST model) throws MiniBrassParseException {
		...
		if (!onlyMiniZinc) {
			...
			sb.append("\nfunction ann: " + pvsBetter + "() = post(" + topLevelInstance.getGeneratedBetterPredicate()
					+ ");\n");
			...
			for (PVSInstance leafInstance : leafInstances) {
				...
				if (leafInstance.getType().instance.getElementType() instanceof MiniZincArrayLike) {
					...
					String equalArrays = "[ sol(" + leafValuation + "[" + indexString + "]) | " + comprehensionString
							+ "] = " + leafValuation + " ";
					equalityBuilder.append(equalArrays);
				} else {
					equalityBuilder.append("sol(" + leafValuation + ") = " + leafValuation + " ");
				}
			}

			String negatedGetNotWorse = "not ( (" + equalityBuilder.toString() + ") \\/ ("
					+ topLevelInstance.getGeneratedNotWorsePredicate() + "))";
			sb.append("\nfunction ann: " + pvsNotWorse + "() = post(" + negatedGetNotWorse + ");\n");
		}
		...
		for (PVSInstance leafInstance : leafInstances) {
			...
			if (!first)
				...
				first = false;
			outputBuilder.append(val + "%s = \\(" + val + ")");
		}
		outputBuilder.append("\\n\"]");
	...
	/**
	 * The overall exported MiniZinc hooks are those keywords that can be easily
	 * accessed in an output statement and/or for search ("postGetBetter",
	 * "topLevelObjective", ...)
	 * 
	 * @param sb
	 * @param model
	...
	private void addExportedOverallMiniZincHooks(StringBuilder sb, MiniBrassAST model,
		...
		if (!onlyMiniZinc) {
			sb.append("function ann:  " + MiniZincKeywords.POST_GET_BETTER + "() = " + pvsBetter + "();\n");
			sb.append("function ann:  " + MiniZincKeywords.POST_NOT_GET_WORSE + "() = " + pvsNotWorse + "();\n");
		}
		...
		if (topLevelOverall != null) { // either case fits
			...
			sb.append("constraint " + MiniZincKeywords.TOP_LEVEL_OBJECTIVE + " = " + topLevelOverall + ";\n");
		}
	...
	private void addPvs(AbstractPVSInstance pvsInstance, StringBuilder sb, MiniBrassAST model)
		...
		if (pvsInstance.isComplex()) {
			...
			if (pvsInstance instanceof CompositePVSInstance) {
				...
				// equals
				String generatedEqualsPredicate = "(" + equalsLeft + ") /\\ (" + equalsRight + ")";
				...
				if (comp.getProductType() == ProductType.DIRECT) {
					...
					comp.setGeneratedBetterPredicate("( (" + leftBetter + ") /\\ (" + rightBetter + ") )");
					comp.setGeneratedNotWorsePredicate("( (" + rightBetter + ") /\\ (" + leftBetter + ") )");
				} else { // lexicographic
					String leftBetter = left.getGeneratedBetterPredicate();
					String rightBetter = right.getGeneratedBetterPredicate();

					comp.setGeneratedBetterPredicate(
							"( (" + leftBetter + ") \\/ ( (" + equalsLeft + ") /\\ " + rightBetter + ") )");
					comp.setGeneratedNotWorsePredicate(
							"( (" + rightBetter + ") \\/ ( (" + equalsRight + ") /\\ " + leftBetter + ") )");
				}
				...
				for (AbstractPVSInstance voter : vi.getChildren()) {
					...
					equalityBuilder.append("(" + voter.getGeneratedEqualsPredicate() + ")");
				}
				equalityBuilder.append(")");
				...
				if (vi.getVotingProcedure().hasNumericObjective()) {
					...
					appendOverall(sb, vi.getVotingProcedure().getObjectiveType(this, vi.getChildren()), vi,
							overallIdent);
					sb.append("constraint " + overallIdent + " = "
							+ vi.getVotingProcedure().getNumericObjective(this, vi.getChildren()) + " ;\n");
				}

				// write count of voters
				String votersCountGenerated = MiniBrassVotingKeywords.VOTER_COUNT + vi.getName();
				sb.append("int: " + votersCountGenerated + " = " + vi.getChildren().size() + ";\n");
				// write identifiers and enums of voters
				StringBuilder namesGenerator = new StringBuilder();
				namesGenerator.append("array[1.." + votersCountGenerated + "] of string: "
						+ MiniBrassVotingKeywords.VOTER_STRING_NAMES + vi.getName() + " = [");
				boolean first = true;
				int enumCounter = 0;
				for (AbstractPVSInstance child : vi.getChildren()) {
					AbstractPVSInstance childDerefed = deref(child);
					String nameStr = "\"" + childDerefed.getName() + "\"";
					if (first)
						first = false;
					else
					...
					// TODO revise
					sb.append("int: " + childDerefed.getName() + " = " + (++enumCounter) + ";\n");
				}
				namesGenerator.append("];\n");
				...
				for (MiniZincBinding binding : model.getBindings()) {
					...
					String genBinding = mznVar + " = " + encodedMetaVar + ";\n";
	...
	/**
	 * Responsible for adding a single, atomic PVS along with parameters, decision
	 * variables, and minisearch predicates
	 * 
	 * @param pvsInstance
	 * @param sb
	 * @param model
	...
	private void addAtomicPvs(AbstractPVSInstance pvsInstance, StringBuilder sb, MiniBrassAST model)
		...
		AtomicPvsInformation api = new AtomicPvsInformation(inst, overallIdent, valuationsArray, subs,
				instanceArguments);
	...
	/**
	 * Problem specific heuristics such as "most-important first" written as an
	 * annotation, basically only links to a corresponding MiniZinc function
	 * 
	 * @param inst
	 * @param sb
	 * @param model
	 ...
	 */
	private void addHeuristics(PVSInstance inst, StringBuilder sb, MiniBrassAST model, String overallIdent,
			StringBuilder instanceArguments, String valuationsArray) {
		...
		if (heuristicFunc != null) {
			sb.append(" = ");
			sb.append(heuristicFunc + "(" + valuationsArray + ", " + overallIdent + ", " + instanceArguments.toString()
					+ ");\n");
		} else {
			sb.append(";\n");
		}
	...
	/**
	 * The soft constraints (EXPR) specified in MiniBrass are translated into the
	 * form "valuations[i] = EXPR"
	 * 
	 * @param inst
	 * @param sb
	 * @param model
	...
	private void addSoftConstraints(PVSInstance inst, StringBuilder sb, MiniBrassAST model, Map<String, String> subs,
		...
		for (SoftConstraint sc : inst.getSoftConstraints().values()) {
			if (pvsType.getElementType() instanceof MiniZincArrayLike) {
				...
				sb.append("constraint forall(" + comprehensions + ") ( " + valuationsArray + "[" + sc.getId() + ","
						+ indexString + "] = " + auxIdent + "[" + indexString + "] );\n\n");
			} else {
				sb.append("constraint " + valuationsArray + "[" + sc.getId() + "] = ("
	...
	/**
	 * We write two MiniSearch predicates, one requiring to "get better" for
	 * domination-based search and one "not get worse" for "only not be dominated"
	 * search (i.e., to find all optima) Not be dominated requires negation so the
	 * predicate better be applicable in a negated context (no free local variables
	 * such as in SPD for deciding the witness)
	 * 
	 * @param inst
	 * @param sb
	 * @param model
	 ...
	 * @param instanceArguments
	 */
	private void addMiniSearchPredicates(PVSInstance inst, StringBuilder sb, MiniBrassAST model,
			Map<String, String> subs, String overallIdent, StringBuilder instanceArguments) {
		sb.append("\n% MiniSearch predicates: \n");
		PVSType pvsType = inst.getType().instance;
		//
		String lastSolutionDegree = "sol(" + overallIdent + ")";
		if (pvsType.getElementType() instanceof MiniZincArrayLike) {
			...
			lastSolutionDegree = "[ sol(" + overallIdent + "[" + indexString + "]) | " + comprehensionString + "]";
		}
		String getBetterString = pvsType.getOrder() + "(" + lastSolutionDegree + ", " + overallIdent + ", "
				+ instanceArguments.toString() + ")";
		String notGetWorseString = pvsType.getOrder() + "(" + overallIdent + ", " + lastSolutionDegree + ", "
				+ instanceArguments.toString() + ")";
		String equalityString = "sol(" + overallIdent + ") = " + overallIdent;
	...
	/**
	 * The decision variables hold the valuations of each soft constraint and the
	 * overall combination using the times operation
	 * 
	 * @param inst
	 * @param sb
	 * @param model
	 ...
	 * @param instanceArguments
	 */
	private void addDecisionVariables(PVSInstance inst, StringBuilder sb, MiniBrassAST model, Map<String, String> subs,
			String overallIdent, String valuationsArray, StringBuilder instanceArguments) {
		sb.append("\n% Decision variables: \n");
		PVSType pvsType = inst.getType().instance;
		...
		IntervalType softConstraintsSet = new IntervalType(new NumericValue(1),
				new NumericValue(numberSoftConstraints));

		MiniZincParType specType = pvsType.getSpecType();
		if (specType instanceof MiniZincVarType) {
			sb.append("array[" + softConstraintsSet.toMzn(inst) + "] of var " + encode(pvsType.getSpecType(), inst)
					+ ": " + valuationsArray + ";\n");
		} else {
			MiniZincArrayLike arrayLike = (MiniZincArrayLike) specType;
			ArrayType arrayType = arrayLike.getArrayType();
			sb.append("array[" + softConstraintsSet.toMzn(inst) + "," + getArrayIndexSets(arrayType, inst) + "] of var "
					+ encode(arrayType.getElementType(), inst) + ": " + valuationsArray + ";\n");
		}
		...
		sb.append(encode(pvsType.getElementType(), inst) + ": " + topIdent + " = " + topElement + ";\n");

		sb.append("constraint " + overallIdent + " = " + pvsType.getCombination() + " (" + valuationsArray + ","
				+ instanceArguments.toString() + ");\n");
	}

	/**
	 * The parameters are generated as MiniZinc parameters and passed to each
	 * application of an ordering predicate etc
	 * 
	 * @param inst
	 * @param sb
	 * @param model
	 ...
	 * @throws MiniBrassParseException
	 */
	private Map<String, String> addAtomicPvsParameters(PVSInstance inst, StringBuilder sb, MiniBrassAST model)
			throws MiniBrassParseException {
		...
		for (PVSFormalParameter pvsParam : inst.getInstanceParameters()) {
			...
			if (pvsParam.getWrappedBy() != null) {
				if (WrapInformation.MINIZINC.equals(pvsParam.getWrappedBy().wrapLanguage)) {
					paramExpression = pvsParam.getWrappedBy().wrapFunction + "(" + paramExpression + ")";
				}
			...
			String def = encode(pvsParam.getType(), inst) + " : " + CodeGenerator.encodeIdent(pvsParam, inst) + " = "
					+ generatedParamExpression + "; \n";
			sb.append(def);
		}
		return subs;
	}
	...
	private String getComprehensionString(ArrayType arrayType, PVSInstance inst) {
		...
		for (PrimitiveType index : arrayType.getIndexSets()) {
			sbBuilder.append(ITER_VARIABLE_PREFIX + i + " in " + encode(index, inst));
			++i;
		}
		return sbBuilder.toString();
	}

	private void appendOverall(StringBuilder sb, MiniZincParType elementType, AbstractPVSInstance inst,
			String overallIdent) {
		if (elementType instanceof MiniZincVarType) {
			sb.append("var " + encode(elementType, inst) + ": " + overallIdent + ";\n");
	...
	private String getArrayDecisionVariable(MiniZincParType elementType, AbstractPVSInstance inst, String arrayIdent) {
		...
		sb.append("] of var " + encode(arrayType.getElementType(), inst) + ": " + arrayIdent);
		return sb.toString();
	}
