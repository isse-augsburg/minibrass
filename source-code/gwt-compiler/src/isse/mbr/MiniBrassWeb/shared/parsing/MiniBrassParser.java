...
import isse.mbr.MiniBrassWeb.shared.model.parsetree.Morphism.ParamMapping;
...
import isse.mbr.MiniBrassWeb.shared.model.types.MultiSetType;
...
import isse.mbr.MiniBrassWeb.shared.model.types.SetType;
import isse.mbr.MiniBrassWeb.shared.model.voting.VotingFactory;
...
public class MiniBrassParser {
	...
	public MiniBrassAST parse(InputStream input) throws MiniBrassParseException {
		...
		try {
			...
			// else {
			// File cousinStd = new File(classPathDir.getParentFile().getParentFile(),
			// "mbr_std");
			// if (cousinStd.exists())
			// mbrStdDir = cousinStd;
			// }
	...
	private Morphism morphismItem() throws MiniBrassParseException {
		...
		if (currSy == MiniBrassSymbol.ParamsSy) {
			...
			if (currSy == MiniBrassSymbol.GeneratedBySy) {
				...
				ExternalMorphism em = ExternalMorphism.create(generationExpression);
				if (em == null)
					throw new MiniBrassParseException(
	...
	private WrapInformation wrappedByAnnotation(PVSType scopeType) throws MiniBrassParseException {
		...
		if (WrapInformation.JAVA.equals(wi.wrapLanguage)) {
			ExternalParameterWrap ew = ExternalParameterWrap.create(wi.wrapFunction);
			if (ew == null)
				throw new MiniBrassParseException(
						"Class " + wi.wrapFunction + " in wrappedBy annotation was not found!");
			wi.setExternalWrap(ew);
		}
		return wi;
	}
	...
	private void includeItem(MiniBrassAST model) throws MiniBrassParseException {
		...
		if (true)
			throw new MiniBrassParseException(
					"Could not find file " + fileName + " in either working directory or MiniBrass standard lib");
		// }
		// }
